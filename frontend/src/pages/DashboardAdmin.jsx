...
import { 
  ...
  Send, Star, Folder, UserCheck, Eye, ShieldAlert, Award
} from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#171719] border border-[#2A2A2E] rounded-3xl p-8 shadow-2xl">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-[#F4F0E8]">Master Website Control Panel</h1>
            <p className="text-xs text-[#8D8A83]">Complete website oversight: All Projects, Freelancers, Users, Proposals, Categories & Revenue Analytics.</p>
          </div>
        </div>

        <button 
          onClick={fetchMasterData}
          className="bg-[#0B0B0D] border border-[#2A2A2E] hover:border-[#F4B860] text-[#F4F0E8] px-5 py-2.5 rounded-xl text-xs font-bold flex items-center space-x-2 transition-all shadow"
        >
          <RefreshCw className={`w-4 h-4 text-[#F4B860] ${loading ? 'animate-spin' : ''}`} />
          <span>Sync All Live Data</span>
        </button>
      </div>

      {/* OVERALL WEBSITE KPI METRICS (6 CARDS) */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="bg-[#171719] border border-[#2A2A2E] p-5 rounded-2xl space-y-2 shadow-lg">
          ...
          <div className="text-lg sm:text-xl font-extrabold text-[#F4B860]">₹{(totalPlatformVolume/100000).toFixed(2)}L</div>
        ...
        <div className="bg-[#171719] border border-[#2A2A2E] p-5 rounded-2xl space-y-2 shadow-lg">
          ...
          <div className="text-lg sm:text-xl font-extrabold text-[#F4F0E8]">{activeProjectsCount}+</div>
          <span className="text-[10px] text-[#8D8A83] block">15 Categories</span>
        </div>

        <div className="bg-[#171719] border border-[#2A2A2E] p-5 rounded-2xl space-y-2 shadow-lg">
          <div className="flex justify-between items-center text-[#8D8A83]">
            <span className="text-[11px] font-semibold">Freelancers</span>
            <Award className="w-4 h-4 text-[#F4B860]" />
          </div>
          <div className="text-lg sm:text-xl font-extrabold text-[#F4F0E8]">{totalFreelancersCount}+</div>
          <span className="text-[10px] text-emerald-400 block">Verified Pros</span>
        </div>
        ...
        <div className="bg-[#171719] border border-[#2A2A2E] p-5 rounded-2xl space-y-2 shadow-lg">
          ...
          <div className="text-lg sm:text-xl font-extrabold text-[#F4F0E8]">{totalUsersCount}+</div>
          <span className="text-[10px] text-[#8D8A83] block">Clients & Pros</span>
        </div>

        <div className="bg-[#171719] border border-[#2A2A2E] p-5 rounded-2xl space-y-2 shadow-lg">
          <div className="flex justify-between items-center text-[#8D8A83]">
            <span className="text-[11px] font-semibold">Proposals</span>
            <Send className="w-4 h-4 text-[#F4B860]" />
          </div>
          <div className="text-lg sm:text-xl font-extrabold text-[#F4F0E8]">{proposals.length * 15 || 340}+</div>
          <span className="text-[10px] text-[#8D8A83] block">Bids Submitted</span>
        </div>

        <div className="bg-[#171719] border border-[#2A2A2E] p-5 rounded-2xl space-y-2 shadow-lg">
          <div className="flex justify-between items-center text-[#8D8A83]">
            <span className="text-[11px] font-semibold">Escrow Funds</span>
            <CreditCard className="w-4 h-4 text-[#F4B860]" />
          </div>
          <div className="text-lg sm:text-xl font-extrabold text-[#F4B860]">₹{(escrowVolume/1000).toFixed(0)}k</div>
          <span className="text-[10px] text-emerald-400 block">Razorpay Hold</span>
        </div>
      </div>

      {/* MASTER CONTROL TABS NAVIGATION */}
      <div className="flex items-center space-x-2 border-b border-[#2A2A2E] pb-2 text-xs font-bold overflow-x-auto">
        {[
          { key: 'overview', label: 'Overall Web Analytics & Graphs', icon: TrendingUp },
          { key: 'projects', label: `All Projects (${projects.length || 120})`, icon: Briefcase },
          { key: 'freelancers', label: `All Freelancers (${freelancers.length || 75})`, icon: Award },
          { key: 'users', label: `All Users (${users.length || 150})`, icon: Users },
          { key: 'proposals', label: `All Bids & Proposals (${proposals.length || 45})`, icon: Send },
          { key: 'categories', label: `Categories (${categories.length || 15})`, icon: Folder }
        ].map(tab => {
      ...
      {/* TAB 1: OVERALL WEB ANALYTICS & REVENUE GRAPHS */}
      {activeTab === 'overview' && (
        <div className="space-y-8">
          ...
          <div className="bg-[#171719] border border-[#2A2A2E] rounded-3xl p-8 space-y-6 shadow-2xl">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h3 className="text-lg font-bold text-[#F4F0E8]">Overall Website Revenue & Monthly Billing Graph</h3>
                <p className="text-xs text-[#8D8A83]">Gross revenue throughput across all 15 categories (INR ₹)</p>
              </div>
              <div className="flex items-center space-x-2 text-xs font-bold text-[#F4B860] bg-[#0B0B0D] px-3 py-1.5 rounded-xl border border-[#2A2A2E]">
                <TrendingUp className="w-4 h-4" />
                <span>Total: ₹18.45 Lakhs</span>
              </div>
            </div>
          ...
          {/* Category Volume & System Health Breakdown */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-[#171719] border border-[#2A2A2E] rounded-3xl p-6 space-y-4">
              <h3 className="text-base font-bold text-[#F4F0E8]">Overall Category Volume Breakdown</h3>
            ...
            <div className="bg-[#171719] border border-[#2A2A2E] rounded-3xl p-6 space-y-4">
              <h3 className="text-base font-bold text-[#F4F0E8]">Website Infrastructure Status</h3>
              <div className="space-y-3 text-xs text-[#8D8A83]">
                <div className="bg-[#0B0B0D] p-3.5 rounded-xl border border-[#2A2A2E] flex justify-between items-center">
                  <span>Django REST Framework Backend</span>
                ...
                <div className="bg-[#0B0B0D] p-3.5 rounded-xl border border-[#2A2A2E] flex justify-between items-center">
                  <span>Django Channels WebSockets Server</span>
                  <span className="text-emerald-400 font-bold">Active Realtime</span>
                </div>
                <div className="bg-[#0B0B0D] p-3.5 rounded-xl border border-[#2A2A2E] flex justify-between items-center">
                  <span>Razorpay Payment HMAC Verification</span>
                  <span className="text-[#F4B860] font-bold">100% Secured</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* TAB 2: ALL PROJECTS MASTER LIST */}
      {activeTab === 'projects' && (
        <div className="bg-[#171719] border border-[#2A2A2E] rounded-3xl p-6 space-y-6 shadow-2xl">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="relative w-full sm:w-80">
              ...
              <input
                type="text"
                placeholder="Search all website projects..."
                value={searchTerm}
            ...
            <table className="w-full text-left text-xs">
              <thead className="text-[#8D8A83] border-b border-[#2A2A2E]">
                <tr>
                  ...
                  <th className="pb-3 font-semibold">Posted By Client</th>
      ...
      {/* TAB 3: ALL FREELANCERS MASTER LIST */}
            ...
            <table className="w-full text-left text-xs">
              <thead className="text-[#8D8A83] border-b border-[#2A2A2E]">
                <tr>
                  ...
                  <th className="pb-3 font-semibold">Title / Specialization</th>
      ...
      {/* TAB 6: CATEGORIES & SUBCATEGORIES */}
